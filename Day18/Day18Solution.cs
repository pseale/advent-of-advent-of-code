...
namespace Day18
{
    ...
    {
        public static (List<string>, int) GetFrames(string input, int frameCount)
        {
            ...
            {
                var nextFrame = Iterate(currentFrame, size);
        ...
        private static char[,] Iterate(char[,] frame, int size)
        {
            var nextFrame = new char[size, size]; // cols, rows

            for (int col = 0; col < size; col++)
            for (int row = 0; row < size; row++)
                nextFrame[col, row] = CalculateNext(frame, size, row, col);

            return nextFrame;
        }

        // hammer out all 8 possible neighbors
        private static char CalculateNext(char[,] frame, int size, int row, int col)
        {
            int litNeighbors = 0;
            for (int x = -1; x <= 1; x++)
            for (int y = -1; y <= 1; y++)
